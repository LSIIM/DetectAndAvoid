...
#include <algorithm>
#include <vector>
#include "../../FuzzyCMeans/fuzzy_clustering.hpp"
#include <string>
#include <cassert>
#include <time.h>
#include <sstream>
...
int main(int argc, char **argv)
{
    ...
    float fuzziness = 4.0;    // initial: 1.1
    float epsilon = 0.01;
    ...
    double frame_height = static_cast<int>(capture.get(cv::CAP_PROP_FRAME_HEIGHT));
    double fps = capture.get(cv::CAP_PROP_FPS);
    double resize_scale = 650.0/frame_height;
    int max_point = 100;

    // cv::VideoWriter writer(filename + "_out_flow.avi", 
    //                        cv::VideoWriter::fourcc('M', 'J', 'P', 'G'), 
    //                        fps, 
    //                        cv::Size(frame_width*resize_scale, frame_height*resize_scale));

    // Verifica se o objeto VideoWriter foi criado corretamente
    // if (!writer.isOpened()) {
    //     std::cerr << "Erro ao criar o arquivo de vídeo para saída!" << std::endl;
    //     return -1;
    // }
    ...
    // for(int i = 0; i < K; i++){
    //     centroides[i].pos.x = frame_width*rand()/(float)RAND_MAX;
    //     centroides[i].pos.y = frame_height*rand()/(float)RAND_MAX;
    //     centroides[i].linhas.x = 200*rand()/(float)RAND_MAX - 100;
    //     centroides[i].linhas.y = 200*rand()/(float)RAND_MAX - 100;
    //     centroides[i].vec_uv.x = 100*rand()/(float)RAND_MAX;
    //     centroides[i].vec_uv.y = 100*rand()/(float)RAND_MAX;
    // }

    vector<Point2f> paths;
    Mat data = (cv::Mat_<float> (2, 5)
    << 
    0.0, 100.0, 15.0, 4.0, 100.0,
    1.0, 200.0, 15.0, 3.0, 150.0);


    SoftC::Fuzzy f (data, number_clusters, fuzziness, epsilon, dist_type, init_type);
    ...
    for(int iter = 0; ; iter++){
        ...
        for(int i = 0; i < good_new.size(); i++){
            ...
            // dataset.at<float>(i,4) = good_new[i].x;
        ...
        // minhas_cores = clusterizar(good_new, paths, uvs, centroides);
        ...
        for(int i = 0; i < good_new.size(); i++){
            ...
            // arrowedLine(frame,p1[i],(p1[i] + uvs[i]),Scalar(0), 2);
            
            // putText(frame,to_string(sqrtf(uvs[i].x*uvs[i].x + uvs[i].y*uvs[i].y)),(p1[i] + uvs[i]*1.25),1,1,Scalar(255,255,255),3);
            // putText(frame,to_string(sqrtf(uvs[i].x*uvs[i].x + uvs[i].y*uvs[i].y)),(p1[i] + uvs[i]*1.25),1,1,Scalar(0),2);
        }
        ...
        for(int i = 0; i < centroides.rows; i++){
            if(sqrtf(centroides.at<float>(i,2)*centroides.at<float>(i,2) + centroides.at<float>(i,3)*centroides.at<float>(i,3)) - vel_media >= 60 ){
                alerta = true;
                arrowedLine(frame,Point(cluster_pos.at<float>(i,0),cluster_pos.at<float>(i,1)),(Point(cluster_pos.at<float>(i,0),cluster_pos.at<float>(i,1)) + Point(centroides.at<float>(i,2),centroides.at<float>(i,3))),Scalar(0,0,200), 2);
            }
        }
        if(alerta)
