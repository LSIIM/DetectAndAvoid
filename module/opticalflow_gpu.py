...
def setup(clusters=5, fps=30, processing_size=(640, 480), max_point=30):
...
def initialize_tracking(frame, context):
    ...
    if context.cuda_available:
        ...
        if len(frame.shape) == 2 or frame.shape[2] == 1:
            context.old_gray_gpu = gpu_resized
        else:
        ...
        context.detection_mask = np.ones((context.processing_size[1], context.processing_size[0]), dtype=np.uint8) * 255
        context.detection_mask_gpu = cv2.cuda_GpuMat()
        context.detection_mask_gpu.upload(context.detection_mask)
    else:
        # CPU fallback
        frame_resized = cv2.resize(frame, context.processing_size)
        if len(frame_resized.shape) == 2 or frame_resized.shape[2] == 1:
            context.old_gray = frame_resized
        else:
...
def process_frame(frame, context):
    ...
    if context.p0 is not None and (context.frame_iter % int(0.5 * context.fps) == 0) and len(context.p0) < context.max_point:
        ...
        if context.cuda_available:
            # Keep the GPU mask synchronized with the CPU mask. GpuMat does not
            # support NumPy comparisons or astype operations.
            context.detection_mask_gpu.upload(context.detection_mask)
            gpu_corners = context.gpu_corner_detector.detect(
                context.old_gray_gpu, mask=context.detection_mask_gpu
            )
            if gpu_corners.size()[0] > 0:
    ...
    if context.cuda_available:
        ...
        gpu_resized = cv2.cuda.resize(gpu_frame, context.processing_size)
        if len(frame.shape) == 2 or frame.shape[2] == 1:
            frame_gray = gpu_resized
        else:
            frame_gray = cv2.cuda.cvtColor(gpu_resized, cv2.COLOR_BGR2GRAY, stream=context.stream)
    else:
        frame_resized = cv2.resize(frame, context.processing_size)
        if len(frame_resized.shape) == 2 or frame_resized.shape[2] == 1:
    ...
    if context.cuda_available:
        ...
        gpu_p0.upload(
            np.asarray(context.p0, dtype=np.float32).reshape(1, -1, 2)
        )
    ...
    context.old_gray = frame_gray.download() if context.cuda_available else frame_gray.copy()
    context.p0 = good_new.reshape(-1, 1, 2)
    context.mask = np.zeros(
        (context.processing_size[1], context.processing_size[0], 3), dtype=np.uint8
    )  # Reset mask
    if context.cuda_available:
        context.mask_gpu = cv2.cuda_GpuMat()
        context.mask_gpu.upload(context.mask)
    ...
    for i, pid in enumerate(good_ids):
        ...
        if pid in context.point_positions and len(context.point_positions[pid]) >= min_frames_check:
            ...
            for j in range(len(recent_vels) - 1):
                ...
                if pid == 172 or pid == 275 and j == len(recent_vels) - 1:
                    print(f"Point ID {pid}: Velocity magnitudes: {mag1:.2f}, {mag2:.2f}")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        cv2.imshow("Optical Flow GPU", frame)
