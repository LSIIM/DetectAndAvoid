...
def main():
    ...
    VIDEO_PATH = r"videos_test/fev_corte_3.mp4"
    
    input_dir, filename = os.path.split(VIDEO_PATH)
    name, ext = os.path.splitext(filename)
    OUTPUT_PATH = os.path.join(input_dir, f"{name}__fp16SkySeg_skip_frames{ext}")
    ...
    # --- Parâmetro de controle para a frequência da segmentação ---
    SEGMENTATION_UPDATE_INTERVAL = 20  # Executar a cada 20 frames
    ...
    try:
        # para rodar yolo em tensor rt usar
        yolo_model = YOLO(YOLO_MODEL_PATH,task="segment")
        # para rodar yolo sem tensorrt usar
    ...
    while True:
        ...
        # --- Condição para executar a segmentação apenas em intervalos ---
        ...
        # --- Usa a última máscara calculada para compor a imagem ---
